    ...
    for (int i = 0; i < grupo.asignados; i++) {
        ...
        if (strncmp(bolilla.equipo.federacion, "UEFA", 3) == 0) {
            if (strncmp(bolilla.equipo.federacion, grupo.bolilla[i].equipo.federacion, 3) == 0) {
                // si es UEFA y ya hab�a encontrado otro en el grupo (bandera)
                if (uefaFlag) {
                    return false;
                // si no, levanto la bandera porque encontr� uno de UEFA
                } else {
                    uefaFlag = true;
                }
            }
        // es cualquier otra federaci�n, comparo que no haya otro de la misma federaci�n
